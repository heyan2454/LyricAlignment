...
## 7b. 口径修正（2026-09-12 第 2 轮重放实验后追加）

- §3 的 A/B 准确表述是"完整清理 vs 最小清理"：raw 评测管线的 selected 阶段相对其 raw 阶段仍调整了 208 个单元（0.72%，全部只动 end，含 32 个负时长→零长钳位），并非字面意义的"无后处理"。
- 好消息：**前向完全可复现** — 同一 identity 在两次独立前向中记录的 raw 阶段差异 0 个单元（最大 0ms），按 identity 复用内容寻址 evidence 的前提成立。
- §3 的建议"拆开两端"已被量化的规则重放取代：最优可部署规则是**只修剪前单元尾端 + 最短时长保护**，见 `reports/progress/20260912_postprocess_policy_replay.md`。
