...
## goal 与长期任务纪律（2026-09-07 Eventizer 冲刺血泪沉淀；对长期/整夜/跨会话任务生效）

> 本节是"完成/科学/报告纪律"与"长期任务执行纪律"的**操作级补充**，每条含本会话实例教训。
> 适用：所有长跑实验、自主续跑、goal/schedule/watchdog 编排。

### A. goal 的正确定位与使用

1. **goal 只承载"当前这段有界自动执行段"，不是常驻容器**。长期研究目标、13 条执行纪律、后台任务
   清单、关键路径——一律放会话外权威文件（各 run root 的 `GOAL_AUTHORITY.md` + `IMPL_STATUS.json` +
   `branch_state.json` + `NEW_FINDINGS.md`），重建/续跑靠**读文件**，不靠记忆、不靠 goal。
2. `create_goal` 时 objective 写"先读 <权威文件路径> 以其为准"的指针 + 纪律摘要；**禁止**把全部
   目标细节塞 objective（易遗漏、随会话膨胀）。
3. 本会话实例：两个 goal 都因高频注入被废弃（见 D-3）。goal 机制在自动轮**无法 pause/resume**
   （报 `requires a direct human turn`）——遇到就用 `complete` 收掉该 goal（后台任务照跑，不受影响），
   不要反复尝试 pause 浪费时间；需要时按 §A-1/2 重建新 goal。

### B. 注入频率自检（每轮第一件事）

1. 每次被注入/每轮开始，先自查：距上次**实质产出**多久？本次有无**新完成事件**（哨兵 flag、
   schedule/watchdog 触发、后台产物更新、用户新消息）？
2. **无新事件 → 本轮零工具调用直接结束**（一两句话说明在等什么即可），把轮让给真实事件。
3. **连续 ≥3 轮零实质产出 = 空转** → 立即 complete 当前 goal（§A-3），绝不靠"每轮零调用"硬扛高频注入。
4. 判定"实质产出"看是否推进了目标（修了代码/出了结果/改了状态/打包），**进度查询不算**——要查进度
   就用 schedule 聚批，别逐轮小查。

### C. 长任务等待：watchdog + 哨兵 + schedule 三件套

1. **watchdog**（`watch_task.sh`）：任何 >10min 后台任务必挂。监视对象用 **log 文件**（持续追加，
   mtime 滚动），**不要监视"跑完才写"的 json/最终产物**——那会在任务健康跑时静默误报
   （实例：B0 跑 2h 期间 watchdog 误报 3 次，全是监视了终态 json）。
2. **完成哨兵**：后台 bash 循环等"全部产物 mtime 新鲜"→ 写 `xxx_done.flag`；主进程只在 flag 出现时处理。
   哨兵超时窗口要给足（≥ 任务最坏时长 ×1.5），B0 全枚举 38 曲 ~1.5-2h，60min 哨兵就超时了。
3. **schedule 是"何时 resume 处理"的触发器，不是空转理由**：设了 schedule 就去做别的/结束本轮，
   schedule 提醒出现才回来处理。
4. 编排脚本（如 `finish_auto.sh` 串"等 flag → 读结果 → 写最终产物"）的等待窗口同样要给足，别比任务
   先超时（实例：finish_auto 50min 超时，B0 52min 才完成，擦肩而过）。

### D. 并行与进程管理

1. **长任务必须并行**：CPU 核够就分片多进程。启动前先确认 CLI 支持分组参数（`--items`/`--max-items`
   切分），**没有就加上再跑**，别启动后才发现参数不存在（实例：B0 并行 4 路首启全崩——argparse 无
   `--items`；B0 单进程 2h → 并行 4 路 40min，快 3×）。
2. **杀进程用精确 pid**：`ps -ef` 先列出再逐个 `kill <pid>`。**禁止 `pkill -f "长命令行子串"`**——会
   匹配到自己所在的 bash -c 命令行自杀（SIGTERM，实例踩了 3 次）。
3. 长任务用 `setsid nohup ... < /dev/null &` 启动（隔离进程组），防后续误杀连带。
4. **多进程写同一输出 = 竞态**：每进程写独立 out/rows 文件，全部完成后再 `cat` 合并（实例：4 组并行
   各写 B0_g{i}.json + rows，合并后 8638 行与官方结果逐位一致）。
5. 启动多进程后**确认都活着**（`ps` 计数 + 各 log 有输出）再离开，别假设成功（实例：setsid 后立即查
   进程数=0，实际 conda 启动延迟；并行首启 4 路全 ERROR 却没及时发现）。

### E. 关键产出多 review（防"做完才发现无效"）

1. 实现后立刻自查：口径是否与计划/权威文件一致？有无把"自定义近似"当"官方"？（实例：B1 taxonomy
   旧版用 GT-frame 判 actionability，GPT review7 指出应查 candidate-frame——数字从"30% 可恢复"修正为
   "22% 粗估"）
2. 聚合/统计代码写完先看**中间量是否合理**（累计值、分母、单位）——实例：export_taxonomy 的
   `n_actionable` per-item 是跨曲累计 bug、B0 `taxonomy_universe_all_rejected` 误=actionable 8638
   （实为 60371）。
3. 不要给"近似/粗分层"起强结论名字（实例：`present_actionable` 曾被叫"可恢复上限"，实为粗估计）。

### F. 报告与结论纪律（承接"完成/科学纪律"）

1. 报告数字一律从结构化 JSON 生成，零手抄；手抄/凭记忆抄 item 列表都算违规（实例：曾手写 val38
   item 分组，后改为从权威 json 读）。
2. 术语与口径冻结：oracle 结果 ≠ 方法提升；`Greedy realizable` ≠ `upper bound`；粗分层 ≠ 上限。
3. 负结果/修正都如实记录到 NEW_FINDINGS（旧结论标 STALE 或直接撤回，不悄悄覆盖）。

### G. 与既有纪律的衔接（本节是补充不是替代）

本节 A-F 是操作级补充，**承接上方既有纪律**（语言/多 agent 编排/长期任务/完成科学报告纪律），完整纪律 = 既有段 + 本节。本节新增的、既有段没有的教训集中在：
- goal 定位与注入频率自检（B 节——既有段只有"禁无意义轮询"的意图，没有可操作的"每轮自检 + ≥3 轮零产出即 complete"）
- watchdog 监视对象选 log 不选终态 json（C-1 实例教训）
- 杀进程禁 pkill 长串自匹配 + setsid 隔离（D-2/3）
- 启动多进程后确认存活（D-5）
- "近似 ≠ 上限"命名纪律 + 累计值/分母 bug 自查（E-2/3、F-2）
