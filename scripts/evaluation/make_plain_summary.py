...
def main() -> None:
    ...
    if argmax and viterbi:
        lines += [f"1. **换一种挑时间的方法**：原来模型逐个字独立挑时间点，"
                  f"改成在\"时间只能往前走\"（术语：单调约束）的前提下整句一起挑。同一批计算结果下，"
    ...
    if gating:
        lines += [f"2. **让模型自己标出不可信的地方**：它自己给出答案时的把握程度（置信度）确实能预示错误。"
                  f"用真实歌曲测（不需要人工标注）：只要人工复核\"最没把握\"的那 10%，"
        ...
        lines += ["2. 挑可疑位置的工具（基于置信度）：状态——结果文件缺失（未跑）。"]
    ...
    lines += ["- **把短句拼成长音频继续训练**：不变好也不变坏（逐首歌对比差 "
              ...
              "后来发现那大半是我自己代码把超出范围的数强行拉回边界（钳位）造成的假象；修正后真实结论是「没什么效果」。"]
    if stress:
