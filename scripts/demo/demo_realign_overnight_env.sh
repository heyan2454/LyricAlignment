...
validate_overnight_inputs() {
  ...
  for path in projector.pt adapter/adapter_model.safetensors adapter/adapter_config.json; do
    [[ -f "$R2_CHECKPOINT/$path" ]] || { echo "ERROR: incomplete R2 checkpoint; missing $R2_CHECKPOINT/$path" >&2; return 2; }
  done
  ...
  for path in model.safetensors config.json tokenizer_config.json; do
    [[ -f "$MODEL_SOURCE/$path" ]] || { echo "ERROR: model snapshot missing $MODEL_SOURCE/$path" >&2; return 2; }
  done
  ...
  export REPO_ROOT PYTHON_BIN MODEL_REVISION MODEL_SOURCE R2_CHECKPOINT M4_LABELS M4_AUDIO_ROOT MIR1K_SUBSET_ROOT DEVICE
}
