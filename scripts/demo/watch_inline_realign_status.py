...
def summary_count(payload: dict[str, Any], *keys: str) -> Any:
    for key in keys:
        if key in payload:
            return payload[key]
    return "—"
...
def render(root: Path) -> str:
    ...
    lines = [
        ...
        f"visuals complete/failed: {summary_count(visuals, 'complete_count', 'completed_item_count')}/{summary_count(visuals, 'failed_count', 'failed_item_count')}",
        f"demo comparison videos complete/failed: {summary_count(renders, 'rendered_item_count', 'completed_item_count', 'complete_count')}/{summary_count(renders, 'failed_item_count', 'failed_count')}",
        f"output size: {human_bytes(directory_size(root))}",
    ]
    failure = (
        live.get("error") or live.get("message")
        or experiment_live.get("error") or experiment_live.get("message")
    )
    if failure:
