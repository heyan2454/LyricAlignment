...
OUT_ROOT="${OUT_ROOT:-/home/hyan/Data/lyricalign/demo_diagnostics/inline_realign_smoke_v4_full_20260728}"
...
RESUME="${RESUME:-0}"                        # 1 resumes the identical run
RETRY_FAILED_ONLY="${RETRY_FAILED_ONLY:-0}"
FROM_STAGE="${FROM_STAGE:-}"
INVALIDATE_STAGE="${INVALIDATE_STAGE:-}"    # comma-separated
RESTART_ITEM="${RESTART_ITEM:-}"            # comma-separated item ids

cd "$REPO_ROOT"
printf '%s\n' \
  "Inline Realign smoke（全机制）" \
  "输出：$OUT_ROOT" \
  "渲染模式：$RENDER_MODE" \
...
command=(
  ...
  --mode smoke --config "$CONFIG" --out-root "$OUT_ROOT" --python-bin "$PYTHON_BIN"
