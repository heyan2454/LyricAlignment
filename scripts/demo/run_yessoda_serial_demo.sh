...
LEFT_CONTEXT_SEC="${LEFT_CONTEXT_SEC:-15}"
RIGHT_CONTEXT_SEC="${RIGHT_CONTEXT_SEC:-15}"
...
spleeter_run() {
  ...
  local generated
  generated="$(find "$tmp" -type f -name vocals.wav -print -quit)"
  [[ -n "$generated" ]] || fail "Spleeter did not produce vocals.wav under $tmp"
  cp -f "$generated" "$output.tmp.wav"
...
prepare_audio() {
  ...
  existing_sha=""
  if [[ -f "$AUDIO_ROOT/vocals.identity.json" ]]; then
    existing_sha="$($PYTHON_BIN - "$AUDIO_ROOT/vocals.identity.json" <<'PY'
import json,sys
from pathlib import Path
try:
    print(json.loads(Path(sys.argv[1]).read_text(encoding='utf-8')).get('mix_sha256',''))
except Exception:
    print('')
PY
)"
  fi
  if [[ -f "$VOCAL_WAV" && "$existing_sha" == "$source_sha" ]]; then
    log "reuse current Spleeter vocal stem"
  else
    log "run Spleeter 2-stem separation"
    spleeter_run "$MIX_WAV" "$VOCAL_WAV"
    "$PYTHON_BIN" - "$AUDIO_ROOT/vocals.identity.json" "$source_sha" "$VOCAL_WAV" <<'PY'
import hashlib,json,sys
from datetime import datetime,timezone
from pathlib import Path
out=Path(sys.argv[1]); vocal=Path(sys.argv[3])
h=hashlib.sha256(vocal.read_bytes()).hexdigest()
out.write_text(json.dumps({
  'schema_version':'yessoda_spleeter_v1',
...
PY
  ...
  log "audio prepared: $MIX_WAV ; $VOCAL_WAV"
}
