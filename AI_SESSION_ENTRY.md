...
3. 机制的**准确表述**（08:05 由置信度×时长交叉修正）：模型的**典型**预测并不收缩
   ...
   **因果方向的证据来自对照臂 A**（未改配比的 600 步续训，留出条目上逐字符配对）：
   只有**长字符的结束点**显著变差（Δ +9.57 ms，z=+2.19），短字符两侧与长字符起始点全部不动
   （Δ ≈ 0）。即"在同一配比上多训会更糟、且只糟在缺样本的那个自由度上"。
   副作用：A 自身在长音上下漂，所以任何"B vs A"的净效应都会**高估** B，必须同时报 B vs 起点。
...
6. 置信度门控可用且**跨歌泛化**：真歌批按歌留一，复核 10% 消除 55% 塌陷（剩余 5.43% vs 原 10.80%）。
   这与 2026-09-12 段在 GTSinger 上独立测得的熵 AUC 0.913 互为印证（今晚真歌 AUC 0.9227）。
   工具：`scripts/evaluation/export_review_gating.py`。
