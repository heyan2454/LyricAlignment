...
5. **DP 解码是今晚唯一到手的免费精度**：全量同一批 logits 下 0.9784 vs argmax 0.9743
   ...
   **更正（17:14）**：这句话在此之前其实不成立 —— `model_identity()` 里扩展身份字段的代码写在 `return` 之后，
   是**永不执行的死代码**（`identity` 也未定义），意味着 DP 批处理的身份字典与官方路径一模一样、
   校验链分辨不出解码器。现已修复并由 3 项契约测试固定（默认路径逐键不变）。
   教训：**"我说过代码已备好"不是证据，测试才是。**
...
7. **两条再平衡路线都已实测收口（今晚的最终结论）**：
   - 数据路径无供给：GTSinger 全库仅 300 个 ≥1s / 70 个 ≥2s；MIR-1K 这份发行没有字符级时间边界
     （要用就得伪标签，已被禁止）；条目级上采样还**自我稀释**（份额增益饱和于 ×2.58）。
   - **B（改采样 factor=8）**：offset_long 相对 A −20.7 ms、相对起点仅 −11.1 ms（McNemar×4=0.085）；
     分段计数显示收益只在 ≥1.5s（新修好 9 / 弄坏 0）、代价只在最短段（0 / 4），
     而结束点缺陷总数 **123 = 起点 123** ⇒ 净零。
   - **C（改目标权重 slot=offset×3，不复制条目）**：offset_long 相对起点 **−5.0 ms（p=0.19 / 0.51 ⇒ 无证据）**，
     缺陷总数 123 → 122（**一个字符**）；相对 A 的 −14.6 ms 只是躲开 A 自身退步（A 退到 129）。
     最短段新弄坏从 B 的 4 降到 2 ⇒ **形状预测命中，但幅度不足**；短条目终态 MAE 反而 +2.51 ms。
   - ⇒ **机制描述是对的（两次独立干预形状都命中），但在现有解码头下用它做再平衡，
     收益低于短条目代价**；按预注册判定 H-C 不成立（线：Δ≤−10 ms 且两检验 p<0.05）。
   - ⇒ 唯一未试的方向是**结构改动**：时间戳头输出「起始 + 时长」参数化，或给结束点单独的解码/回归分支
     ——这是新的工程预算，需先立项再跑臂（证据：`docs/status/20260914_c_verdict.md`）。
8. **热启动改标签语义的连带损伤规律（结构臂 §6h/§6i）**：第二槽位换成时长语义 + 完整 LR，
   会把共享特征上"没被改动的起始点"砸坏（>0.2 s 的字符占比从基线 0.7% 涨到 13.8%），
   表现是**判别性被打散**（熵 0.75→2.79、p_top1 0.69→0.20），不是错得自信；标签变换本身有单测排除。
   ⇒ 规则：**监控没被改动那个槽位的熵/p_top1**，几百步就能发现，不必等整轮跑完；
   ⇒ 修法是把 LR 降一半（备用配置已备，只在预注册判据触发时启用）。
8a. **停后台脚本要停对进程（21:21 我自己制造了重复 controller）**：
   `setsid nohup bash X.sh &` 会留下一条 `bash -c '<我的启动命令>'` 包装进程和一条真正的
   `bash X.sh`（父进程为 1）。我只杀了前者 ⇒ 脚本仍在等 ⇒ 又启动一次 ⇒ **两条等待中的实例**，
   GPU 空闲时会并发跑两个任务。
   ⇒ 规则：① 杀之前先 `ps -eo pid,ppid,etime,args | grep F "X.sh"` 看清父子；
      ② 要杀的是 `bash <脚本路径>` 那条（ppid=1），必要时连它的父壳一起杀；
      ③ 杀完必须用 `ps -eo args | grep '^bash /'|grep -c X.sh` 重新数，确认每条链**恰好 1 个实例**。
8b. **陈旧完成标记会骗停新任务（今晚两次）**：`*.done` / `*.done.failed` 哨兵是跨运行残留的。
   第一次：concat20 的 `.done.failed` 是早前崩溃留下的假故障，我差点当成真失败；
   第二次：19:13 一次启动失败留下的 `.done.failed` 让新的同步数对比链**立刻判定"对照臂已失败"并全部跳过**。
   ⇒ 规则：**等待型脚本只认"比自己启动时间更新"的标记**（`stat -c %Y ≥ START_EPOCH`），
      旧标记**改名保留**（`.stale-from-<时间>-attempt` + 一行说明），不删除；
   ⇒ 启动任何新的自动链之后，**必须看一行它的日志**确认它进入了等待而不是立刻结束。
