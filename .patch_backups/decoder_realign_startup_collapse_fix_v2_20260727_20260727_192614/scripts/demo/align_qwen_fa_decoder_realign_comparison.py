...
def branch_args(args: argparse.Namespace, decoder_kind: str) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        serial_control_decoder_kind="official",
...
def main() -> int:
    ...
    request = {
        ...
        "design": {
            ...
            "serial_control_decoder": "official for all four branches",
            "branches": BRANCHES,
    ...
    try:
        ...
        if not trajectory_match:
            raise RuntimeError(
                "official-controlled trajectories diverged between official and raw branches; "
                "decoder comparison is not fair"
            )
        trajectory_hash = SERIAL.canonical_hash(official_trajectory)
