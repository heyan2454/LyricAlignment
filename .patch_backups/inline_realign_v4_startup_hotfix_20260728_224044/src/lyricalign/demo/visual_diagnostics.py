...
def draw_track(
    ...
    assigned, lane_count = assign_lanes(visible)
    lane_height = 0.32
    for row, lane in assigned:
        ...
        if x1 < x0 - 1e-9:
            ax.add_patch(FancyArrowPatch((x0, y), (x1, y), arrowstyle="-|>", mutation_scale=8,
                                         linewidth=1.6, linestyle="--", color=color, zorder=4))
            ax.text((x0+x1)/2, y+0.06, text, ha="center", va="bottom", fontsize=font_size,
                    color=color, clip_on=True, zorder=5)
        elif x1 <= x0 + 1e-9:
            ax.vlines(x0, y-0.11, y+0.11, linewidth=2.2, color=color, zorder=4)
            ax.text(x0, y+0.13, text, ha="center", va="bottom", fontsize=font_size,
                    color=color, clip_on=True, zorder=5)
        else:
            width = x1 - x0
            ax.add_patch(Rectangle((x0, y-0.095), width, 0.19, facecolor=color, edgecolor=color,
                                   linewidth=0.55, alpha=0.43, zorder=3))
            # Text is always rendered; lane packing prevents most collisions.
            ax.text((x0+x1)/2, y, text, ha="center", va="center", fontsize=font_size,
                    color=character_color(row_index(row), 1.0), clip_on=True, zorder=5)
...
def render_inconsistency(
    *, output: Path, tracks: list[tuple[str, list[dict[str, Any]]]], title: str,
    ...
    grid = fig.add_gridspec(3, 1, height_ratios=[2.2, 1.1, 1.5], hspace=0.30)
    ax1=fig.add_subplot(grid[0]); ax2=fig.add_subplot(grid[1]); ax3=fig.add_subplot(grid[2])
    ...
    ax1.set_ylabel("全局时间（秒）"); ax1.set_title("歌词序号—起止时间")
    ax1.legend(ncol=min(3,len(names)), fontsize=8); ax1.grid(alpha=0.25)
    start_array=np.array(starts, dtype=float); end_array=np.array(ends, dtype=float)
    ...
    for threshold in (0.16,0.24,0.50): ax2.axhline(threshold,linewidth=0.8,linestyle=":",label=f"{threshold:.2f}s")
    ax2.set_ylabel("最大差（秒）"); ax2.set_title("同一歌词单位在不同层次/窗口中的最大差")
    ax2.legend(ncol=5,fontsize=8); ax2.grid(alpha=0.25)
    ...
    image=ax3.imshow(delta,aspect="auto",interpolation="nearest",cmap="coolwarm",vmin=-limit,vmax=limit,
                     extent=[indices[0]-0.5 if indices else 0,indices[-1]+0.5 if indices else 1,len(names)-0.5,-0.5])
    ax3.set_yticks(range(len(names)),names); ax3.set_xlabel("全局歌词序号"); ax3.set_title("偏差热力图（灰色为空缺）")
    image.cmap.set_bad("lightgray"); fig.colorbar(image,ax=ax3,label=heatmap_label,pad=0.01)
    fig.suptitle(title,fontsize=15,fontweight="bold")
    output.parent.mkdir(parents=True,exist_ok=True); fig.savefig(output,dpi=145,bbox_inches="tight"); plt.close(fig)
    return {"path":str(output),"track_count":len(tracks),"unit_count":len(indices),"max_start_spread_sec":float(np.nanmax(start_spread)) if len(indices) else None,"max_end_spread_sec":float(np.nanmax(end_spread)) if len(indices) else None}
