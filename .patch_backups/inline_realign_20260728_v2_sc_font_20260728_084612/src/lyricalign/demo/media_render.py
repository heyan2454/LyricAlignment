...
def _register_matplotlib_font(font_path: Path) -> str | None:
    """Register a concrete font file in the current Matplotlib process.

    Matplotlib's persistent font cache can predate a newly installed system font.
    ``fontManager.addfont`` makes the matched file immediately available without
    requiring users to delete ``~/.cache/matplotlib``.
    """
    try:
...
def detect_font(preferred: str) -> str:
    """Resolve a requested family or font file and register it for Matplotlib.

    Debian's ``NotoSansCJK-*.ttc`` can be exposed by fontconfig as
    ``Noto Sans CJK JP`` even when ``Noto Sans CJK SC`` was requested.  Returning
    the concrete face name is correct for both Matplotlib and libass; registering
    the matched file avoids stale Matplotlib-cache fallbacks to DejaVu Sans.
    """
    requested_path = Path(preferred).expanduser()
    if requested_path.is_file():
        registered = _register_matplotlib_font(requested_path.resolve())
        return registered or requested_path.stem
    ...
    matched_file = Path(lines[0]) if lines else None
    family = lines[1].split(",")[0].strip() if len(lines) > 1 else preferred
    if matched_file is not None and matched_file.is_file():
        registered = _register_matplotlib_font(matched_file.resolve())
        if registered:
            return registered
    return family or preferred
