...
def main() -> int:
    ...
    for ordinal, item in enumerate(manifest, 1):
        ...
        try:
            ...
            if not b2_path.is_file():
                raise FileNotFoundError(b2_path)
            ...
            for token in requested:
                if token == "RAW_B2":
                    payload = read_json(item_root / "branches" / "B2_30_silence_official" / "alignment.raw.json")
                    name = "Raw"
                else:
            ...
            experimental = [
                ...
                ("Immediate realign", item_root / "experimental_alignments" / "R1_immediate_inline" / "alignment.json"),
            ...
            for name, path in experimental:
                ...
                if payload:
                    tracks.append((name, canonical_rows(payload)))
            ...
            atomic_json(output_root / "visual_analysis.json", payload)
    ...
    summary = {
        "schema_version": "inline_realign_visual_batch_v1", "item_count": len(manifest),
