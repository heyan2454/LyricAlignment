...
def render_set(
    *, paths: list[Path], labels: list[str], visual: Path | None, audio: Path,
    ...
    if len(paths) != 4 or not all(path.is_file() for path in paths):
        return None
    return render_alignment_comparison(
...
def main() -> int:
    ...
    for ordinal, item in enumerate(demo_items, 1):
        ...
        try:
            ...
            if main is None:
                missing = [str(path) for path, _ in main_pairs if not path.is_file()]
                raise FileNotFoundError(f"main comparison inputs missing: {missing}")
            stable_pairs = [
            ...
            behavior = BEHAVIOR.render_behavior_video(
                ...
                visual_source=visual, audio_track=audio, output_path=render_root / "behavior_current.mp4",
    ...
    payload = {
        "schema_version": "inline_realign_demo_render_batch_v2_multiway_and_behavior",
