...
- 顶替机制要额外花评估次数，所以新增 `l2_budget` 作为**真正的成本上限**（默认 = `l2_max`，
  即不顶替；建议 `2 * l2_max`）；
- `l2_per_round: N` + `l2_every: M`：二层**只在 M 步的轮次边界开闸**，每次收本轮"最好的 N 个挑战者"
  （按便宜层的 UCB 排序）。这对应"底层的新冠军上来挑战二层"；如果改成"每个存档点都能录取、但每轮
  限 N 个"，那么本轮开始时领先的那个会先吃掉名额，把本轮后出现的更强候选挡在外面；
- 三层的名额是**上限 + 可顶替**：名额满后，挑战者必须"看起来不弱于当前最弱的决赛者"才值得多花一次
  评估，所以晚到的强候选能把早到的弱候选顶掉。底层第一永远进得来（它是 UCB 最高者，必过闸门）；
- 顶层（全集）每个轮次最多收 `l3_top_k` 个新的（含二层当前第一），同一个候选同一层只评一次。

新 config 建议写法：`l2_max: 16, l2_budget: 24, l2_every: 500, l2_per_round: 1`
（12000 步 → 24 个轮次 × 1 个挑战者，正好用满预算；实测在真实的 51 个 L1 点上，
 该设置会依次录取 500/900/1150/550/850 … 即"每轮把当前最有希望的未升格候选收上来"）。
