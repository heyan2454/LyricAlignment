...
## 3g-4. **§3g-3 的预测被实测否证**（08:13，A 臂逐字符配对）—— 这条比预测更重要
07:36 我登记的噪声地板是"600 步续训在 ≥2s 上预期移动 <0.1 pp"。用同一批 808 条留出条目
做 A vs 起点 的**逐字符配对**后：

| 侧 | n | Δ（A − 起点，正=更差） | z | 超差率 |
|---|---|---|---|---|
| 起始点·短字符 | 5,481 | −0.23 ms | −1.64 | 0.57% → 0.53% |
| 结束点·短字符 | 5,479 | +0.00 ms | 0.00 | 1.39% → 1.41% |
| 起始点·长字符 | 938 | +0.53 ms | +0.68 | 1.49% → 1.60% |
| **结束点·长字符** | 938 | **+9.57 ms** | **+2.19** | 5.01% → 5.54% |
| 长字符合计 | 938 | +10.28 ms | +2.32 | 6.18% → 6.82% |
| 全体 | 6,415 | +1.37 ms | +2.06 | 2.53% → 2.62% |

⇒ **噪声地板的预测在"长字符结束点"上被否证**：那里不是 <0.1 pp，而是显著变差（z≈2.2–2.3）。
⇒ 但**位置特异性完美**：短字符两侧与长字符起始点都纹丝不动（Δ ≈ 0，z 不显著），
   退化精确落在**长字符的结束点**——正是整晚诊断出的那个位置。
⇒ 科学含义：这不再是"暴露少 ⇒ 误差大"的**横断面相关**，而是**操作性证据**：
   在未改变的配比上多训 600 步，会让稀有类别变得更差、且只变差在它缺样本的那个自由度上。
   机制（模型把区间长度拉向训练分布众数）由此获得因果方向的支持。

### 对 B 判定的修改（现在就写下，不看结果再改）
1. 因为 A 自身在长字符结束点上**向下漂移**，`B vs A` 会**高估** B 的收益；
   所以必须同时报告 **B vs 起点（uniform-12000）** 与 **B vs A**，并以 **B vs 起点** 为"是否真的修好"的主读法，
   以 **B vs A** 为"配比改变的净效应"。两个都报，不择其一。
2. §3g-2 的预测带（≥2s 6.81%、1–2s 3.68%）本来就是相对起点，保持不变。
3. 主判定量精确到**长字符结束点**这一侧（n=938，配对 SE 4.4 ms），而不是"长音整体"。
