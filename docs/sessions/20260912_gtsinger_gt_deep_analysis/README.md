...
# 第 32 轮：指标能分辨多小的差？——格点余量与"差值可归因性"

- 代码：`label_noise_ceiling.metric_stability / compare_with_stability / gap_artifact_bound`
- 入口：`scripts/evaluation/{run_label_noise_ceiling,report_metric_stability}.py`；测试 3 项（本文件累计 7 项）
- 产物：`runs/20260912_label_noise_ceiling/{STABILITY,GAP_ARTIFACT}.json`、
  `reports/progress/20260912_metric_stability.md`、`results/by_run/20260912_metric_stability/metrics.json`

## 核心：落后方"只差不到一个格点"的单元数，就是差值的不可归因区

| 对比 | 容差 | 差值 | 格点余量可造 | 可归因？ | 扎实差异单元 |
|---|---|---:|---:|---|---:|
| r1 vs r0（GTSinger） | 100ms | **+18.00pp** | 4.82pp | **是** | 1,784 |
| r2 vs r1（GTSinger） | 100ms | +2.51pp | 2.47pp | 勉强是 | 268 |
| r2 vs r1（GTSinger） | 50ms | +2.78pp | **9.96pp** | **否** | 200 |
| r2_full vs r1_full（MIR-1K） | 100ms | +0.29pp | 3.54pp | **否** | 36 |
| r2_full vs r2_ood（MIR-1K） | 100ms | −0.69pp | 2.85pp | **否** | 28 |
| r1_full vs r0_raw（MIR-1K） | 100ms | +16.56pp | 8.40pp | **是** | 236 |

## 单系统脆弱性（同一格点同向偏移能把命中率推动多少）
GTSinger：hit@50 **±42.1pp**（刀尖判定 84.2%）、hit@100 **±36.7pp**（73.4%）、
hit@200 ±3.2pp（6.3%）、hit@250 ±2.5pp（5.1%）。
MIR-1K：hit@50 ±47.9pp、hit@100 ±40.5pp、hit@200 ±2.0pp、hit@250 ±0.7pp。
长音层 GTSinger hit@100 ±28.5pp（刀尖 56.9%）。
⇒ **50/100ms 上的绝对命中率不是稳定量**（中位误差 40ms < 80ms 格点，密度压在判定线上）；
200ms 之后才进入"格点无关"的稳定区。

## 硬规则（已写入清单第 18 条）
比较两系统在同一标签上的 hit@tol 差值时**必须同时报** `max_spurious_gap_pp`；
小于该界的差值一律记为**不可归因**，不得写进结论或用于选型。

## 对本会话既往结论的追溯限定（不删原文，只加限定）
- 第 15 轮「联合求解改善首单元 +2.78pp」与本轮 `r2_vs_r1@50ms`（+2.78pp，界 9.96pp）同量级
  ⇒ 该 pp 差**不可作为独立证据**；同轮的 AUC/MAE 型结论不受影响；
- 第 4/5/9/10 轮中凡 hit@100 差值 <3pp 的比较（含集成成员排序、选择器对比）应视为**未分离**；
- 索引 A3 的「r1→r2 ≤0.5pp」由定性表述升级为**可计算的界**（本轮支撑）；
- 不受影响：lift/AUC/相关性型结论（第 15/19/20/26 轮）、结构非法率与阶段归因（第 12–16 轮）。

自查：报告生成器有两处取数错误（把 r2/r1 的差值印成 r1/r0；总跨度与 ±半界混用），已修正；
另第五次踩"中文串内嵌 ASCII 双引号"语法错误——本会话已四次，改为**写代码时直接用「」**而非事后修。

---

# 第 33 轮：SAFE 带边格点脆弱（主线 gate 直接相关）+ 机器生成会话交接页

- 代码：`label_noise_ceiling.band_edge_stability()`；产物 `BAND_EDGE.json`、
  `reports/progress/20260912_metric_stability.md` §2b、`docs/status/20260912_session_handoff.md`
- 生成器：`scripts/evaluation/make_session_handoff.py`（全部数字取自 `results/by_run/*/metrics.json`，不手抄）

## 一、把可归因性规则回应用到 detector_v2 冻结分带

测试方法：把带边整体移动**一个 80ms 格点**（标签/预测的最小可能改动），看带内占比摆多大。

| 面板 | 单元 | SAFE≤100ms 带内 | **一格摆幅** | 250ms 带内 | 摆幅 | 判读 |
|---|---:|---:|---:|---:|---:|---|
| gtsinger_all | 30,600 | 80.37% | **73.4pp** | 88.81% | 5.1pp | 100ms 脆弱 |
| **gtsinger_prod(r2\|vocal\|windowed)** | 2,550 | 88.04% | **77.9pp** | 94.98% | **2.7pp** | 100ms 脆弱 |
| gtsinger_r0 | 10,200 | 67.53% | 65.9pp | 77.65% | 8.5pp | 100ms 脆弱 |
| gtsinger_r1 | 10,200 | 85.53% | 76.3pp | 93.80% | 4.2pp | 100ms 脆弱 |
| mir1k_r2_full | 2,035 | 91.45% | **81.0pp** | 98.62% | 1.4pp | 100ms 脆弱 |
| m4_all（差系统） | 134,538 | 5.21% | 17.3pp | 28.17% | **22.8pp** | 两带边都脆弱 |

结论：
1. **SAFE ≤0.100s 不能作为产品决策的唯一依据** —— 在最差的面板上带内占比一格能摆 65–81pp
   （例如 88.04% → 约 10% 或 96%），因为好系统的中位误差 40ms **小于** 80ms 格点；
2. **UNSAFE ≥0.250s 在好系统上稳定**（摆幅 1.4–8.5pp）⇒ 需要"可下判断的带"时用 250ms，
   100ms 只作报告口径；
3. **副产品**：250ms 摆幅本身是**质量体检量**（生产视图 2.7pp、MIR-1K 1.4pp、M4 22.8pp）。

## 二、会话交接页（可给主线直接引用）
`docs/status/20260912_session_handoff.md`：三张表（已确认 / 已否证或降级 / 预算与决策），
每行给出 canonical 来源与机器摘取的 headline；末尾列"立即可用的三件产物"和"仍待办"
（含两件需 GPU：训练侧、能产生新候选的重解码；以及三项 HEAD 自带失败测试刻意未动）。
...
# 第 35 轮：复核 gate 的 SAFE 边工作点（把 100ms 推到 200ms，三个指标同时变好）

- 代码：`label_noise_ceiling.gate_operating_points()`；入口 `scripts/evaluation/{run,report}_gate_operating_points.py`
- 产物：`runs/20260912_gate_operating_points/GATE_OPERATING_POINTS.json`（≈36K）、
  `reports/progress/20260912_gate_operating_points.md`、`results/by_run/20260912_gate_operating_points/metrics.json`
- 数据：冻结 detector_v2 的 8 个 `LABELS.jsonl`（972,330 个已标单元）；UNSAFE 固定 ≥0.250s；格点 0.08s
- **语义**：这是**标签侧天花板**（假设 gate 完美预测真值误差），不是探测器实际覆盖率（第 5 轮已测 UNSAFE 召回仅 3–5%）
...
⇒ **SAFE ≤0.10s → ≤0.20s：自动通过 +8.7pp、稳健判定 +64.1pp、待复核 9.72%→0.99%**；
当前 100ms 边是**既少放行、放行得又不确定**（≤100ms 放行中 71.7% 处在距带边不足一格的刀尖区）。
留出 test 片（100,722 单元）与全量合并口径的形状一致（100ms 稳健率 28.8%、200ms 92.5%）⇒ 非子集巧合。

## 建议（写进清单第 21 条，本会话未改生产 gate）
1. SAFE 取 **≤0.200s（两格缓冲）**；UNSAFE 保持 **≥0.250s**（第 33/34 轮证其格点稳定）⇒ 200–250ms 成窄复核带；
2. 100ms 保留为**报告口径**，不再作为自动放行依据；
3. 若嫌 200ms 偏松：按域分别设边（产品真实伴奏普通话 vs 清唱录音室），依据第 21 轮"伴奏端点偏晚 / 清唱偏早"，比单一全局边更合理；
4. 实际收益仍受探测器精度限制：本轮给出的是带边定义的天花板，下一步应把 gate 的**实测**覆盖率在同一工作点下重算。

自查：本轮两处自己写的取数/措辞错误并修正 —— ① `per_file` 用 `path.name` 取文件名导致 8 个文件同名，
"排除坏跑"误排全部（units=0），改 `path.parent.name`；② 报告里把"稳健率 28.3%"写成了"刀尖占比"（应为补数 71.7%）。

---

# 第 36–37 轮：实际 gate 能兑现多少天花板（按 SAFE 边的工作点实测）

- 代码：`redecode_budget.band_policy_table()`（阈值只在拟合片选，评估片测，天花板同片重算可直减）
- 入口：`scripts/evaluation/{run,report}_gate_band_policy.py`；测试 4 项
- 产物：`runs/20260912_gate_band_policy/GATE_BAND_POLICY.json`（12K）、
  `reports/progress/20260912_gate_band_policy.md`、`results/by_run/20260912_gate_band_policy/metrics.json`
- 分数只能用**边界末端熵**（GTSinger 与 M4 都记录的唯一置信度信号；MIR-1K 自然面板没有后验列）
- 切分：GTSinger 按 item 分半互拟合/互评估；M4 用 train 拟合、validation 评估、test 仅作冻结转移

## GTSinger（人工真值，评估片真 unsafe ≈11%）
| 误放预算 | SAFE 边 | 实测放行 | 天花板放行 | 余量 |
|---|---|---:|---:|---:|
| 5% | ≤100ms | **A→B 不可行**（无阈值满足预算）／B→A 仅 29.6% | 80.0–80.0% | — |
| 5% | ≤160ms | 66.0% | 86.2% | 20.2pp |
| 5% | ≤200ms | **76.3%** | 87.3% | **11.0pp** |
| 2% | ≤160ms | 7.9% | 86.2% | 78.3pp |
| 2% | ≤200ms | 31.8% | 87.3% | 55.5pp |

⇒ **三条硬结论**：
1. **≤0.100s 这一边不可操作**：同样的 5% 误放预算，一侧折叠根本无解、另一侧只能放行 30%
   ⇒ 100ms 边的阈值**不可迁移**（而 200ms 边两折叠分别放行 76.3% / 72.7%，差 3.6pp）；
2. **≤0.200s 下熵单信号能兑现天花板的 ~87%**（76.3% / 87.3%），余量仅 **11pp**
   ⇒ 与第 27 轮"融合 0.850 vs 只用熵 0.845"一致：**剩余余量主要在新证据（训练/窗口），换分数吃不掉**；
3. **M4 长时序上该 gate 在所有边、两种预算下都不可行**（评估片真 unsafe 69.8%、test 转移片 82.3%）
   ⇒ 普遍错的数据 + 近乎无信息的熵（第 30 轮 ρ≈0）= 任何低误放预算都只能几乎不放行；
   这不是 gate 的实现缺陷，而是它在该质量层的正确行为（应当报警而非放行）。

## 阈值转移的正常代价（§2）
12 个可操作组合中 **6 个**评估片误放率高于拟合片值，最大 5.88%（拟合片 1.76%）
⇒ 预算只是拟合片上的保证；若要评估片也有保证，需 GroupKFold 分位数阈值或预留校准片。

## 自查
- `band_policy_table` 成功分支再次漏 `available` 标志（与第 29/31 轮同一类错误），已修并有回归测试；
- M4 掩码与子帧长度不一致（`Item wrong length 134538 instead of 113618`）⇒ 先取子帧再算掩码；
- §2 首版"最多 4.8%"只统计了一个折叠（另一折实测 5.47%）⇒ 改为跨折叠取真实最大值；
- 第七次踩"中文串内嵌 ASCII 双引号"，且我的自动修复脚本又吃掉了一个 f-string 结束引号
  ⇒ 教训：**这类错误只能用 edit 工具精确定点修，不要写自动"修复"脚本**。

---

# 第 38 轮：gate 的分数阶梯——每加一个免费信号能多兑现多少天花板

- 代码：`run_gate_band_policy.py` 的 `add_score_ladder()`（五档分数，同一拟合/评估切分）
- 入口：`scripts/evaluation/report_gate_score_ladder.py`（报告）；产物沿用 `GATE_BAND_POLICY.json`
- 切分：GTSinger 按 item 分半互拟合/互评估；M4 用 train 拟合、validation 评估、test 仅冻结转移

## 结果（5% 误放预算，GTSinger 两折叠；每格 = 评估片实测放行率）
| 分数档 | ≤100ms | ≤160ms | ≤200ms |
|---|---|---|---|
| R1 只用边界熵 | **不可行** / 仅 29.6% | 66.0% / 68.3% | 76.3% / 72.7% |
| R2 +起止倒序 | 不可行 | 65.9% / 70.7% | 76.2% / 75.8% |
| **R3 +间隙残余（等权分位）** | 不可行 | 78.1% / 75.6% | **85.0% / 80.6%** |
| R4 第 27 轮融合分位 | 不可行 | 68.2% / 66.0% | 75.9% / 75.1% |
| R5 oracle（仅参照） | 83.1% / 79.8% | 88.3% / 87.6% | 90.8% / 87.6% |
| 零风险天花板 | 80.7% / 80.0% | 86.2% / 85.2% | 87.3% / 87.2% |

四条结论：
1. **间隙残余是本轮唯一有增量的成分**：R3 vs R1 在 200ms 边 **+8.7pp / +7.9pp**，把余量从 11.0pp 压到 **2.3pp**；
2. **倒序没有增量**（R2≈R1）⇒ 与第 32 轮"倒序预示塌陷而非端点精度"完全一致，作为端点 gate 成分可剔除；
3. **第 27 轮的融合分位（R4）没兑现 R3 的收益**：它让近似常量的倒序与大面积缺失的间隙一起平均、权重被 NaN 稀释
   ⇒ 修正式：**对含缺失值的免费信号用等权分位平均，不用「含缺失即跳过」的融合**；
4. **R5 oracle 贴着天花板**（GTSinger headroom −3.5~+0.2pp，M4 +0.12~+0.36pp 且误放 0%）
   ⇒ 框架自洽；oracle 略超天花板是正常现象（阈值定在拟合片，评估片实现带小误放；天花板是"零风险放行"上限）。
5. **M4 上 R1–R4 全档不可行**（三个边 × 两种预算都无解），而零风险天花板本身只有 ~23%
   ⇒ 问题不在"分数不够好"，而在**这条链路产物质量不支持自动化**。

**必须保留的限定**：R3 的收益完全依赖 `gap_over_core`，而它**跨语料复现未通过**（第 30/31 轮：M4 ρ=−0.06、AUC 0.506）。
⇒ R3 目前只是 **GTSinger 口径内的上限证据，不得上线**；要落地必须先在有真值的伴奏数据上重新证明。

## 自查（本轮四处）
- `edge_view` 调用约定被我用混（第一参数应是 `by_rung` 而非 `by_budget`）⇒ 天花板被取成 0.0%、余量出现 -76pp；
  统一为 `(fold, rung, budget, edge)` 并全量改调用点；
- 报告里留了两个占位垃圾函数（`cell` 抛 RuntimeError、`if False else` 分支）⇒ 删除；
- M4 掩码与子帧长度不一致（134538 vs 113618）⇒ 先取子帧再算掩码；
- 第八次踩"中文串内嵌 ASCII 双引号"语法错误，且第九次发生在自动修复脚本自身吃引号的问题上
  ⇒ 已在生成器文件头写 NOTE 提醒后续编辑者只用「」。

---

# 第 39 轮：把学到的放行率搬到 33 首真实歌（无真值下的结构审计）

- 入口：`scripts/evaluation/{run,report}_gate_application_to_batch.py` / `report_gate_batch_application.py`
- 产物：`runs/20260912_gate_batch_application/GATE_BATCH_APPLICATION.json`、
  `reports/progress/20260912_gate_batch_application.md`、`results/by_run/20260912_gate_batch_application/metrics.json`
- 口径：**绝对熵阈值不跨域**（第 27 轮）⇒ 按 GTSinger 学到的**放行率**做批内分位匹配；
  本批无真值 ⇒ 只审计**结构安全**（零长/超长/重叠/起点回退），不作精度声明。零前向、零 GPU。

## 数字（整批 13,735 单元：零长 16.28%、结构非法 16.72%，与第 12 轮同口径）
| | 放行率 | 放行集合零长 | 拦下集合零长 | 零长捕获率 |
|---|---:|---:|---:|---:|
| R1 只用边界熵 | 76.30% | **5.10%** | **52.26%**（lift +47.1pp） | **76%**（用 23.7% 的复核量） |
| R3 +间隙残余 | 85.01% | 8.74% | 59.06% | 54%（用 15.0% 的复核量） |

三条结论：
1. **边界熵在产品批上仍然带来真实结构信息**：把 76% 的零长单元集中进 23.7% 的复核队列
   ⇒ gate 不是装饰，它的排序在生产数据上有效（这是无真值条件下能得到的最强正面证据）；
2. **R3 的新增放行是有毒的**：相对 R1 新增放行的 1,490 个单元里 **35.7% 是零长**（整批 16.3%，**2.2×**）
   ⇒ **无真值的结构代理独立否证了间隙残余的迁移性**，与第 30/31 轮跨语料复现失败完全一致 ⇒ R3 不得上线；
   且本批间隙特征只覆盖 12.7%，R3 的排序变化主要来自这不到 1/8 的单元；
3. **单一全局阈值 = 按语言分配复核预算**：R1 下普通话放行 **94.1%**、粤语 72.5%、英语 66.1%、**日语 27.1%**
   ⇒ 如果不希望"日语几乎全部进复核"，必须按语言分别标定阈值（这是策略选择，不是数据问题）。

复核队列最集中的歌（I See Fire 3%、初音未来的消失 7%、p.h 16%、冬之花 23%、皱鳃鲨 23%）
与第 27 轮重解码队列**同源同序** ⇒ 两份独立构造的清单互相印证；仍只是建议顺序，不是判定。

自查：我自己的阈值分位方向写反（`quantile(1-rate)` 应为 `quantile(rate)`），
首版"实现 23.7% vs 目标 76.3%"就是这个 bug（还导致 newly_accepted 集合为空、出现 NaN）；
已修正重跑。另第十次踩中文串内嵌 ASCII 引号。

---

# 第 40 轮：全局阈值 vs 按语言等放行率 vs 按语言等风险（三策略前沿）

- 代码：`structural_compliance.policy_audit()`（无真值的放行/拦下审计，已从脚本上移到 src）
- 入口：`scripts/evaluation/run_gate_language_calibration.py`
- 产物：`runs/20260912_gate_language_calibration/LANGUAGE_CALIBRATION.json`
- 三策略在**同一总放行率**下比较；等风险策略用结构标签反推目标 ⇒ 是"要做到等风险得花多少"的**前沿参照**，不是可部署规则

整批结构非法率按语言差异极大：**日语 49.3% / 英语 22.7% / 粤语 15.1% / 普通话 6.6%**。

在总放行 76.3% 这一档：
| 策略 | 复核量 | 非法捕获率 | 队列语言构成（复核率） |
|---|---:|---:|---|
| **全局分位** | 23.70% | **76.0%** | 粤 27.5 / 普 **5.9** / 英 33.9 / 日 **72.9** |
| 按语言**等放行率** | 23.71% | **55.0%**（−21.0pp） | 四语言一律 23.7% |
| 按语言**等风险**（放行集合非法率对齐全局的 8.11%） | 30.14%（+6.4pp） | 63.4% | 粤 45.6 / 普 **1.6** / 英 44.9 / 日 **99.2** |

三档（70/76.3/85%）形状一致：等放行率损失 **16.3~21.6pp** 捕获率，等风险多花 **~5–6pp** 复核量且捕获仍低于全局。

## 决策建议
1. **不要按语言等放行率**：它在"同等复核预算下捕获缺陷"与"风险均等"两个轴上都不如全局（严格被支配）；
2. 若产品要求**各语言交付风险相当**，就显式采用**等风险**标定，并接受它比全局多花约 6pp 复核量、
   且会把复核负担几乎全部压到日语（放行率 0.8%）——**这应当是一个被知情做出的选择**，
   而不是今天全局阈值那种"顺带按语言分配预算"的隐性效果；
3. 若只追求**单位复核量的缺陷捕获**，保持全局阈值即可；普通话 94.1% 自动放行是这一目标的自然结果。
...
# 第 42–43 轮：零长度字的画像——「整段丢了」，且后处理把塌陷放大 11 倍

- 代码：`raw_degeneracy_forensics.context_features()`（局部语境 + **非循环**的歌曲级密度）、
  `zero_runs()`（连续段长度）、`zero_length_profile()`（对比 + 五分位 + 位置 + AUC）
- 入口：`scripts/evaluation/{run,report}_zero_length_profile.py`；测试 +2（该文件共 7 项）
- 产物：`runs/20260912_zero_length_profile/ZERO_LENGTH_PROFILE.json`（≈10K）、
  `reports/progress/20260912_zero_length_profile.md`、`results/by_run/20260912_zero_length_profile/metrics.json`

## 核心数字（33 首真歌交付线，13,735 单元）
| 量 | 值 |
|---|---:|
| 零长度字 | **2,236**（16.28%） |
| 连续段数 | **734** |
| 最长连续段（交付） | **251 字**（初音未来的消失） |
| 最长连续段（原始阶段） | **22 字** |
| ≥5 长度段中的零长占比 | 交付 **59.3%** vs 原始 23.2% |
| 零长 >30% / >50% 的歌 | **6 首 / 3 首** |
| 歌曲级密度 ↔ 零长比例 | Spearman **−0.0699**（否证密度假设） |
| 最后一字 / 第一字的零长率 | 18.2% / 6.1% |

## 结论
1. **损坏是块状的，不是零散的**：59.3% 的零长字处于长度 ≥5 的连续段，最长一段 251 个字（整首歌 561 字）；
2. **后处理把塌陷放大约 11 倍**（22 → 251，≥5 段占比 23.2% → 59.3%）⇒ 优先堵"整块钉到窗口起点/预钳位"
   这条放大链，而不是逐字修补；
3. **不是"歌词太密"**：歌曲级密度与零长比例几乎无关（−0.07）；
4. **集中在少数歌**：6/33 首吃掉大部分损坏，最健康四首**全是普通话**（1.2–1.9%），
   但普通话也有一首进最差榜（画下灯塔水母，135 连续段）；
5. **末尾字单独值得处理**（18.2%，与"末单元缺右侧上下文"一致）。

## 本轮自查（一处指标陷阱）
"局部窗口密度"在本轮**是循环论证**：窗口含零长单元时时长塌缩 ⇒ 密度虚高（原始阶段中位数顶到裁剪值
5000 字/秒）。已把该量改名为 `local_window_chars_per_sec` 并在代码注释/报告里明确标注
**只能当"扎堆证据"、不能当原因**，同时新增**歌曲级密度**（用整首歌时间轴跨度算，零长单元无法干扰）
作为非循环判据，其结论是密度假设被否证。
另修：驱动脚本漏 `import pandas as pd`（导致连续两次"空输出"，浪费了一轮）。

---

# 第 44 轮：超长塌陷的归因——`fixed` 阶段把整段钉到同一时刻

- 代码：`raw_degeneracy_forensics.degenerate_run_lineage()`（对同一批下标逐阶段还原退化数/不同起点数/跨度）
- 入口：`scripts/evaluation/{run,report}_degenerate_run_lineage.py`；测试 +2（该文件共 9 项）
- 产物：`runs/20260912_degenerate_run_lineage/DEGENERATE_RUN_LINEAGE.json`、
  `reports/progress/20260912_degenerate_run_lineage.md`、`results/by_run/20260912_degenerate_run_lineage/metrics.json`

## 取证（交付线长度 ≥50 字的 6 个塌陷块，逐阶段）
| 歌 | 块长 | `raw` 退化 / 不同起点 / 跨度 | `fixed` | `selected` | `final` |
|---|---:|---|---|---|---|
| 初音未来的消失 | 251 | 133/251、123 起点、108.7s | **251/251、2 起点、67.9s** | 同 fixed | 同 fixed |
| I See Fire | 199 | 107/199、79 起点、79.2s | **199/199、1 起点、0.0s** | 同 fixed | 同 fixed |
| 画下灯塔水母 | 135 | 80/135、40 起点、68.2s | **135/135、1 起点、0.0s** | 同 fixed | 同 fixed |
| 冬之花 | 113 | 67/113、40 起点、102.1s | **113/113、2 起点、9.4s** | 同 fixed | 同 fixed |
| 初音未来的消失 | 97 | 56/97、38 起点、71.7s | **97/97、1 起点、0.0s** | 同 fixed | 同 fixed |
| I See Fire | 63 | 34/63、28 起点、54.6s | **63/63、1 起点、0.0s** | 同 fixed | 同 fixed |

三条结论：
1. **放大全部发生在 `fixed` 一步**：每个块的逐阶段净增里，`fixed` 之外的步骤**净增都是 0**；
2. **机制可验证**：`fixed` 把整段压缩到极少甚至唯一的时间点（79 起点/79.2s → 1 起点/0.0s）；
3. **没有一个超长块在 `raw` 就是全塌陷**（0/6）⇒ 塌陷是**下游做出来的**，且对 `raw` 尚可用的信息是**破坏性**的。

总量：交付线 2,236 个零长字中 **641 个（28.7%）位于 16 个"起点完全相同"的块**（最大 199 字），
其余多段块呈 2 个时间点（两次窗口边界各钉一次）。

## 行动含义
- **修法优先级**：先修 `fixed`（= 生产名 `processor_decoded`）这一步的整块钳位/钉锚逻辑；
  它把"局部塌陷"变成"整段塌陷"，是第 43 轮"放大 11 倍"的唯一贡献者；
- 之后再谈重解码：原始阶段这些块仍有一半以上单元可用、时间跨度真实，说明**信息原本是有的**。
...
# 第 46 轮：修掉塌陷要付多少代价？——一刀切掉 4pp，定向修几乎免费

- 代码：`monotone_repair.repair_monotone_min_duration()`（全时间线重排，反例）与
  `repair_targeted_blocks()`（**只修塌陷/倒序块**，推荐）；`repair_stage_frame()` 支持传入修复函数
- 入口：`scripts/evaluation/{run,report}_shadow_repair_value.py`；测试 +8（`test_monotone_repair.py`）
- 产物：`runs/20260912_shadow_repair_value/SHADOW_REPAIR_VALUE.json`、
  `reports/progress/20260912_shadow_repair_value.md`、`results/by_run/20260912_shadow_repair_value/metrics.json`
- **只读实验**：影子修复从不写回产品路径

## GTSinger（人工词级真值，30,600 单元）
| 时间线 | 中位误差 | 零长度 | hit@100 | hit@200 | hit@250 |
|---|---:|---:|---:|---:|---:|
...
1. **一刀切是错的**：零长度归零，但中位误差 40→46ms、hit@200 掉 **3.98pp**（把本来正确的字也重排了）；
2. **定向修复几乎免费且更好**：零长度 0%，中位误差不变，hit@100/200/250 **+1.02/+1.09/+1.01pp**（对现交付值），
   对不修补的原始 argmax 为 −0.73/+0.52/+0.59pp；
3. 原理：塌陷块内部本来就没有可用时间信息，在健康邻居之间摊开**不破坏任何原有信息**。

## 真歌批（无真值，结构口径）
| 变体 | 零长度 | 结构非法 | 同一起点块（≥5） | 最大块 |
|---|---:|---:|---|---:|
| 现交付值 | 16.28% | 16.72% | 16 块 / 641 字 | 199 |
| 影子·全时间线重排 | 0.00% | 0.36% | 0 | 0 |
| 影子·只修塌陷块 | **0.00%** | 13.62% | **0** | **0** |

- 定向修复后残余非法（13.62%）**全部继承自原始阶段**（原始：重叠 11.36%、起点回退 6.57%、合计 24.35%；
  修复后：重叠 9.44%、回退 4.16%），**没有新增任何一类违规**；
- 既有联合合法化求解作用在现交付值上能把非法率降到 0，但**零长度仍为 16.28%** ⇒ **两处修复互补**：
  正确顺序是**先定向修复塌陷 → 再走既有联合合法化**。

## 更新后的修法建议（清单第 26 条）
1. **不要**用"全时间线最小时长重排"替换上游修补（实测掉 ~4pp）；
2. **要**只重排"同一起点连续 ≥2 字"或"结束不晚于开始"的块，块边界由健康邻居决定；
3. 顺序：定向修复 → 联合合法化 → 交付前"同一起点块"告警；
4. 预期收益：hit@200 **+1.09pp**、hit@250 **+1.01pp**，同时在真歌批上消除 **16.28%** 的零长度字。
...
# 第 49 轮：改前/改后的批次自检预览（33 首逐首）

- 入口：`scripts/evaluation/{run,report}_shadow_batch_audit.py`
- 产物：`runs/20260912_shadow_batch_audit/SHADOW_BATCH_AUDIT.json`、
  `reports/progress/20260912_shadow_batch_audit.md`、`results/by_run/20260912_shadow_batch_audit/metrics.json`
- 做法：读 33 首已落盘的逐字记录（13,735 行），按 `raw_with_targeted_repair` 离线重建时间线，
  与现交付值用同一套结构检查对比；**不改动任何产品文件**
...
- **33 首逐首都降到 0.0%**（原来最高一首 88.1%）；
- 重叠/起点回退升高**不是改动引入的缺陷**，而是影子时间线**尚未经过产品既有的后处理**
  （现交付值重叠仅 0.10%，正是那一步清掉的）⇒ **上线位置必须在既有后处理之前**，
  且**不得再叠加**治重叠/整体合法化（第 47 轮实测 −3.60/−4.32pp）。

---

# 第 50 轮：评测集污染度——我们过去报的精度里有多少是流水线塌陷

- 代码：`label_noise_ceiling.degeneracy_contamination()`；入口 `scripts/evaluation/{run,report}_eval_contamination.py`
- 产物：`runs/20260912_eval_contamination/EVAL_CONTAMINATION.json`、
  `reports/progress/20260912_eval_contamination.md`、`results/by_run/20260912_eval_contamination/metrics.json`

| 评测集 | 预测器 | 没有位置的字 | 误差<0.2s | 去掉后（＝修复上限） | 这些字占失败 | 最长块 |
|---|---|---:|---:|---:|---:|---:|
| MIR-1K | base_qwen_raw_v1 | **20.15%** | 38.72% | **46.52%**（+7.80pp） | 30.3% | 8 |
| MIR-1K | r0_raw / r1 / r2_full / r2_ood / r2_seed | 2.36 / 0.44 / 0.59 / 0.25 / 0.49% | 86.63 / 97.79 / 97.99 / 97.94 / 98.23% | 87.97 / 98.12 / **98.47** / 98.08 / 98.62% | 12.1 / 15.6 / 24.4 / 7.1 / 22.2% | 0 |
| GTSinger | r0 / r1 / r2 | 5.69 / 4.12 / 3.53% | 75.41 / 91.96 / 94.27% | 77.88 / 94.68 / **96.50%** | 15.2 / 36.6 / **41.1%** | 0 |

三条结论：
1. **两个有人工真值的评测集都被同一缺陷污染**，量级从 0.25% 到 20.15%（未适配基础模型最严重）；
2. **「去掉退化单元后的值」＝修复能给该指标带来的上限**；因此修复后的复测**必须同比**，
   不得把向上限靠拢的涨幅算作训练/模型的进步；
3. **失败中结构性占比可观**：GTSinger r2 **41.1%**、r1 36.6%；MIR-1K r2_full 24.4%
   ⇒ 引用历史精度时应并列「去掉退化单元」的值（第 31 轮定的纪律，现在有具体数字）。

---

# 第 51 轮：去掉塌陷单元后，长音收尾的跨域反向偏置仍在（且更强）

- 入口：`scripts/evaluation/{run,report}_bias_excluding_degenerate.py`
- 产物：`runs/20260912_bias_excluding_degenerate/BIAS_EXCLUDING_DEGENERATE.json`、报告、metrics
- 偏置＝预测−真值（负＝切早，正＝切晚）；口径与第 21 轮一致（GTSinger 全检查点 official 30,600 单元）

| 面板 | 子集 | 长音收尾中位 | 偏晚比例 |
|---|---|---:|---:|
| GTSinger 全检查点 | 含塌陷 | −40.0ms | 23.9% |
| | **去掉塌陷** | **−48.0ms** | 22.1% |
| | 只看塌陷 | **+10.0ms** | **75.0%** |
| GTSinger 仅 r2 | 含塌陷 / 去掉 | −14.5 / −16.0ms | 33.8% / 32.0% |
| MIR-1K r2_full | 含塌陷 / 去掉 | +32.4 / **+34.0ms** | 74.1% / 74.8% |

**结论**：一负一正、符号相反 ⇒ **"全局偏移校正不可跨域迁移"继续成立，而且依据更硬**——
塌陷单元自己偏晚（+10ms、75% 偏晚），把它们混进来是在**掩盖**真实的提前偏置，
去掉后提前量反而从 40ms 涨到 48ms。

**口径更新**：第 21 轮报的 −40ms/+32.4ms 是含塌陷的混合值；今后引用改用**去退化版（−48ms / +34ms）**，
或两处并列。判定不变。
...
# 第 53 轮：承重数字复核——长音「答案不在候选里」不是塌陷造成的假象

- 入口：`scripts/evaluation/{run,report}_unreachable_recheck.py`
- 产物：`runs/20260912_unreachable_recheck/UNREACHABLE_RECHECK.json`、报告、metrics
- 口径与第 19/21 轮一致：**精确同格**；四种过滤（全部／去掉交付端塌陷／去掉解码端倒序／两者都去）

| 口径 | r0 | r1 | r2 | r0→r2 改善 |
|---|---:|---:|---:|---:|
| 全部单元（旧口径） | 70.8% | 27.5% | 21.7% | +49.2pp |
| 去掉交付端塌陷 | 70.2% | 26.5% | 20.5% | +49.7pp |
| 去掉解码端倒序 | 70.8% | 27.5% | 21.2% | +49.6pp |
| **两者都去掉（推荐引用）** | **70.2%** | **26.5%** | **20.5%** | **+49.7pp** |

- **旧数字被逐位复现**（70.8 / 27.5 / 21.7）⇒ 重算与历史同一把尺子；
- **去掉塌陷与倒序后结论不变、还略好**：r2 仍有 **20.5%** 的长音收尾真值不在前两名候选内
  ⇒ **这不是选择/后处理能碰到的部分**，"重解码预算在长音层与随机无异"的依据同样更稳；
- **训练路线的依据更强而不是更弱**：干净子集上 r0→r2 的改善 **+49.7pp**（旧口径 +49.2pp）；
- 影响面小：塌陷在长音里占比 r0 5.7% / r1 4.1% / r2 3.5% ⇒ 不推翻任何结论，只是把引用口径换成更干净的版本。

今后引用建议：「长音端点约 40% 的 GT 不在 top-2 内」→ 保持；
「训练把长音不可达率从 70.8% 压到 21.7%」→ 可写为**去塌陷版 70.2% → 20.5%**（并注明口径）。
